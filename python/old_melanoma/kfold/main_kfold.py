...
def main():
    ...
    for fold, (idxT, idxV) in enumerate(skf.split(np.arange(15))):
        ...
        for epoch in range(0, int(EPOCHS[fold] / EPOCH_PER_STEP)):
            print('#' * 25)
            print('#### FOLD', fold + 1)
            print('#### Image Size %i with EfficientNet B%i and batch_size %i' %
                  (IMG_SIZES[fold], EFF_NETS[fold], BATCH_SIZES[fold]))
            print('EPOCH ITERATION %i' % epoch)
            ...
            validation_data = get_dataset(files_valid, augment=False, shuffle=False, repeat=False,
                                          dim=target_size_)
            count_data = count_data_items(files_train) / BATCH_SIZES[fold]
        ...
        model.save(MODEL_PATH + MODEL_NAME + 'efnet_' + str(EFF_NETS[fold]) + '_fold-%i-%ix%i.model' % (
            fold, IMG_SIZES[fold], IMG_SIZES[fold]))
