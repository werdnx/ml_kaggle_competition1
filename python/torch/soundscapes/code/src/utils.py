...
# Augmentation
def process_file(file_path, train=True):
    ...
    sound_formatted = torch.zeros([WINDOW, 1])  # temp_data accounts for audio clips that are too short
    if sound_data.numel() <= WINDOW:
        sound_formatted[:sound_data.numel()] = sound_data[:]
    else:
        rand_index = np.random.randint(0, high=(sound_data.numel() - WINDOW))
        sound_formatted[:] = sound_data[rand_index:rand_index + WINDOW]

    sound_formatted = sound_formatted.permute(1, 0)
    return sound_formatted


def process_sound(sound_data, train=True):
    if train:
        sound_data = augment(samples=sound_data, sample_rate=SAMPLE_RATE)
    # sound_data = mixer(augmented)
    # downsample the audio to ~8kHz
