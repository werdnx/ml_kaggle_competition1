...
sshpass -p "werdn" ssh werdn@$R_HOST 'DEST=/media/3tstor/ml/IdeaProjects/ml_kaggle_competition1; cd $DEST/docker; docker run --name kaggle_container -v $DEST/input:/input -v $DEST/output:/output -v $DEST/python:/python ml_kaggle1:1.0'